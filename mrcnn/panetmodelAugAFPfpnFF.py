...
class PyramidROIAlign1(KE.Layer):
    ...
    def __init__(self, pool_shape,index, **kwargs):
        super(PyramidROIAlign1, self).__init__(**kwargs)
    ...
    def call(self, inputs):
        ...
        roi_level2 = log2_graph(tf.sqrt(h * w) / (224.0 / tf.sqrt(image_area)))
        roi_level2 = tf.minimum(2, tf.maximum(
            2, 4 + tf.cast(tf.round(roi_level2), tf.int32)))
        roi_level2 = tf.squeeze(roi_level2, 2)

        # Loop through levels and apply ROI pooling to each. P2 to P5.
        pooled2 = []
        box_to_level2 = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.where(tf.equal(roi_level2, level))
            ...
            pooled2.append(tf.image.crop_and_resize(
                feature_maps[i], level_boxes, box_indices, self.pool_shape,
                method="bilinear"))

        # Pack pooled features into one tensor
        pooled2 = tf.concat(pooled2, axis=0)

        # Pack box_to_level mapping into one array and add another
        # column representing the order of pooled boxes
        box_to_level = tf.concat(box_to_level2, axis=0)
        ...
        pooled2 = tf.gather(pooled2, ix)

        # Re-add the batch dimension
        shape = tf.concat([tf.shape(boxes)[:2], tf.shape(pooled2)[1:]], axis=0)
        pooled2 = tf.reshape(pooled2, shape)
        #3
        roi_level3 = log2_graph(tf.sqrt(h * w) / (224.0 / tf.sqrt(image_area)))
        roi_level3 = tf.minimum(3, tf.maximum(
            3, 4 + tf.cast(tf.round(roi_level3), tf.int32)))
        roi_level3 = tf.squeeze(roi_level3, 2)

        # Loop through levels and apply ROI pooling to each. P2 to P5.
        pooled3 = []
        box_to_level3 = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.where(tf.equal(roi_level3, level))
            level_boxes = tf.gather_nd(boxes, ix)
            ...
            # which is how it's done in tf.crop_and_resize()
            # Result: [batch * num_boxes, pool_height, pool_width, channels]
            pooled3.append(tf.image.crop_and_resize(
                feature_maps[i], level_boxes, box_indices, self.pool_shape,
                method="bilinear"))

        # Pack pooled features into one tensor
        pooled3 = tf.concat(pooled3, axis=0)

        # Pack box_to_level mapping into one array and add another
        # column representing the order of pooled boxes
        box_to_level = tf.concat(box_to_level3, axis=0)
        box_range = tf.expand_dims(tf.range(tf.shape(box_to_level)[0]), 1)
        box_to_level = tf.concat([tf.cast(box_to_level, tf.int32), box_range],
                                 axis=1)
        ...
        ix = tf.nn.top_k(sorting_tensor, k=tf.shape(
            box_to_level)[0]).indices[::-1]
        ix = tf.gather(box_to_level[:, 2], ix)
        pooled3 = tf.gather(pooled3, ix)

        # Re-add the batch dimension
        shape = tf.concat([tf.shape(boxes)[:2], tf.shape(pooled3)[1:]], axis=0)
        pooled3 = tf.reshape(pooled3, shape)
        #4
        roi_level4 = log2_graph(tf.sqrt(h * w) / (224.0 / tf.sqrt(image_area)))
        roi_level4 = tf.minimum(4, tf.maximum(
            4, 4 + tf.cast(tf.round(roi_level4), tf.int32)))
        roi_level4 = tf.squeeze(roi_level4, 2)

        # Loop through levels and apply ROI pooling to each. P2 to P5.
        pooled4 = []
        box_to_level4 = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.where(tf.equal(roi_level4, level))
            level_boxes = tf.gather_nd(boxes, ix)
            ...
            # which is how it's done in tf.crop_and_resize()
            # Result: [batch * num_boxes, pool_height, pool_width, channels]
            pooled4.append(tf.image.crop_and_resize(
                feature_maps[i], level_boxes, box_indices, self.pool_shape,
                method="bilinear"))

        # Pack pooled features into one tensor
        pooled4 = tf.concat(pooled4, axis=0)
        ...
        ix = tf.nn.top_k(sorting_tensor, k=tf.shape(
            box_to_level)[0]).indices[::-1]
        ix = tf.gather(box_to_level[:, 2], ix)
        pooled4 = tf.gather(pooled4, ix)

        # Re-add the batch dimension
        shape = tf.concat([tf.shape(boxes)[:2], tf.shape(pooled4)[1:]], axis=0)
        pooled4 = tf.reshape(pooled4, shape)
        #5
        roi_level5 = log2_graph(tf.sqrt(h * w) / (224.0 / tf.sqrt(image_area)))
        roi_level5 = tf.minimum(5, tf.maximum(
            5, 6 + tf.cast(tf.round(roi_level5), tf.int32)))
        roi_level5 = tf.squeeze(roi_level5, 2)

        # Loop through levels and apply ROI pooling to each. P2 to P5.
        pooled5 = []
        box_to_level5 = []
        for i, level in enumerate(range(2, 6)):
            ix = tf.where(tf.equal(roi_level5, level))
            level_boxes = tf.gather_nd(boxes, ix)
            ...
            # which is how it's done in tf.crop_and_resize()
            # Result: [batch * num_boxes, pool_height, pool_width, channels]
            pooled5.append(tf.image.crop_and_resize(
                feature_maps[i], level_boxes, box_indices, self.pool_shape,
                method="bilinear"))

        # Pack pooled features into one tensor
        pooled5 = tf.concat(pooled5, axis=0)

        # Pack box_to_level mapping into one array and add another
        # column representing the order of pooled boxes
        box_to_level = tf.concat(box_to_level5, axis=0)
        box_range = tf.expand_dims(tf.range(tf.shape(box_to_level)[0]), 1)
        box_to_level = tf.concat([tf.cast(box_to_level, tf.int32), box_range],
                                 axis=1)
        ...
        ix = tf.nn.top_k(sorting_tensor, k=tf.shape(
            box_to_level)[0]).indices[::-1]
        ix = tf.gather(box_to_level[:, 2], ix)
        pooled5 = tf.gather(pooled5, ix)

        # Re-add the batch dimension
        shape = tf.concat([tf.shape(boxes)[:2], tf.shape(pooled5)[1:]], axis=0)
        pooled5 = tf.reshape(pooled5, shape)
        if (self.index == 2):
            return pooled2
        elif(self.index == 3):
            return pooled3
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    # 为一个图像生成检测目标。子采样建议并生成目标类ID、边界框增量和每个掩码。
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        # Bottom-up Layers
        N3 = KL.Add(name="fpn_n3add")([P3, KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3), strides=(2, 2),padding="SAME", name='fpn_n2conv')(P2)])
        N4 = KL.Add(name="fpn_n4add")([P4,KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3),  strides=(2, 2),padding="SAME", name='fpn_n4conv')(N3)])
        N5 = KL.Add(name="fpn_n5add")([P5,KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (3, 3),  strides=(2, 2),padding="SAME", name='fpn_n5conv')(N4)])
